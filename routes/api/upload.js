...
exports.update = async function(req, res) {
  Dataset.model.findById(req.params.id).exec(function(err, dataItem) {
    ...
    dataItem.getUpdateHandler(req).process(data, async function(err) {
      ...
      const targetBlobName = `${dataService.sanitizeStr(dataItem.file.filename.replace(originalFileExtension, ""))}__${dataService.sanitizeStr(fieldsite.name)}__${dataService.sanitizeStr(data.dateOfReading)}`;
