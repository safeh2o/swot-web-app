...
exports.notifyFileUpload = async function (filename, recipientEmail, country, project, fieldsite, user, dataset) {
  ...
  const uri = `${process.env.ANALYZER_URL}/analysis?filename=${filename}&recipient=${recipientEmail}&country=${dataService.getIdentifier(country)}&project=${dataService.getIdentifier(project)}&fieldsite=${dataService.getIdentifier(fieldsite)}&dataset=${dataset.id}`;
