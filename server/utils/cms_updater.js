...
const uri = process.env.MONGO_DB_CONNECTION_STRING.replace(
	`/${process.env.CMS_SOURCE_DB}?`,
	"/?"
);
...
exports.updateOtherPages = async function (page) {
	if (
		process.env.CMS_DESTINATION_DBS &&
		process.env.CMS_DESTINATION_DBS !== ""
	) {
...
async function updateOtherPage(page, dbName) {
	return MongoClient.connect(
		...
		async (err, client) => {
			...
			const newValues = {
				$set: { content: page.content, state: page.state },
			};
...
async function updateBlog(client, srcBlog, destBlog, dbname_dest) {
	...
	if (srcBlog.categories && srcBlog.categories.length) {
		...
		for (let i = 0; i < srcBlog.categories.length; i++) {
			...
			const translation = await translateModel(
				client,
				process.env.CMS_SOURCE_DB,
				dbname_dest,
				collectionName,
			...
			if (translation.dest) {
				...
				const newCat = await postcategories_dest.insertOne(
					translation.src
				);
...
async function translateModel(
	...
	const doc_src = await collection_src.findOne(
		{ _id: id },
		{ projection: { _id: 0 } }
	);
	const doc_dest = await collection_dest.findOne({
		[keyName]: doc_src[keyName],
	});
