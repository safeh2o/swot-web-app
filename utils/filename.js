...
exports.swotAnalysisFilenameGenerator = function (file, index, callback) {
  nameFunctions.randomFilename(file, index, function(f, filename) {
    ...
    const originalFilename = file.originalname.replace(extension, "").replace(/[^0-9a-z]/gi, ''); //remove non alphanumeric characters
    callback(null,  `${originalFilename}_${filename.substr(0,4)}${extension}`);
  });  
}
